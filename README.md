...
| `checker` | object | Настройки проверки VPN-ссылок (опционально, см. ниже) |
...
Для проверки работоспособности VPN-ссылок перед обновлением конфигурации можно
включить через секцию `checker` в `config.yaml`. При включении каждая VPN-ссылка
проверяется запуском sing-box с минимальным конфигом и выполнением HTTP-запроса
через SOCKS5 прокси. Ссылки, не прошедшие проверку, исключаются из обновления.

**Алгоритм проверки:**

1. Парсинг VPN URL в outbound-конфигурацию
2. Выбор случайного свободного TCP-порта на `127.0.0.1`
3. Создание минимального конфига sing-box:
   - SOCKS5 прокси на `127.0.0.1:<случайный_порт>`
   - Outbound VPN-сервера
   - Direct outbound для fallback
4. Запуск `sing-box run -c /tmp/singbox-check-*.json`
5. Ожидание готовности (проверка подключения к динамическому порту)
6. HTTP-запрос через SOCKS5 прокси к тестовым URL
7. Если ответ 2xx — ссылка работает, иначе — пропускается
8. Остановка sing-box (SIGINT) и удаление временного конфига

**Динамические порты**

Каждая проверка запускает sing-box на случайном свободном порту, чтобы
избежать конфликтов при параллельных запусках и не занимать фиксированные
порты (`10808`, `10809` и др.). Порт выбирается через временный
`net.Listen("tcp", "127.0.0.1:0")`, сразу закрывается и передаётся в
конфиг sing-box и в `runner.Start`.

```yaml
checker:
  enabled: true
  # sing_box_path: "sing-box"
  # tmp_directory: "/tmp/vpnconfig"
  # timeout: "10s"
  # urls:
...
| Поле | Тип | Описание |
|------|-----|----------|
| `enabled` | bool | Включить проверку VPN-ссылок. По умолчанию `false`. |
| `sing_box_path` | string | Путь к утилите sing-box. Если не указан — `sing-box` из PATH. |
| `tmp_directory` | string | Директория для временных файлов. По умолчанию OS temp directory. |
| `timeout` | duration | Максимальное время проверки одной ссылки. По умолчанию `10s`. |
| `urls` | array | URL для проверкиconnectivity. По умолчанию `["https://www.gstatic.com/generate_204"]`. |

**Примечание:** проверка каждой ссылки занимает несколько секунд (запуск
sing-box + HTTP-запрос). Общее время обновления увеличивается пропорционально
количеству ссылок. Рекомендуется использовать `timeout: "10s"` или меньше.
...
├── checker/
│   └── checker.go               # Проверка VPN-ссылок через локальный sing-box
├── runner/
│   └── runner.go                # Управление процессом sing-box
├── portgen/
│   └── portgen.go               # Генератор случайных свободных TCP-портов
